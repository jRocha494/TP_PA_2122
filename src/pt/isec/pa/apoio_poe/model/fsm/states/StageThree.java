...
public class StageThree extends StateAdapter {
    public StageThree(AppContext ac, DataLogic dl) {
        super(ac, dl);
        ac.setCloseStatus("Stage3", false);
    }

    @Override
    public AppState getState() { return AppState.PROPOSAL_ATTRIBUTION_STAGE_THREE; }

    @Override
    public String getStage() { return "Third Stage - Proposal Attribution"; }
    ...
    public boolean closeStage(){
        if(dl.hasEveryStudentWithApplicationsBeenAssigned() && ac.isStageClosed("Stage2")) {
            ac.setCloseStatus("Stage3", true);
    ...
    public boolean manuallyAssign(String student, String proposal) {
        try{
            Proposal pToAssign = dl.getProposal(proposal.split(" ")[0]);
            Student sToAssign = dl.getStudent(Long.parseLong(student));

            if(pToAssign == null || sToAssign == null)
                return false;
